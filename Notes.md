...
1. The first channel is to keep track of in-progress streaming goroutines. Upon receiving SIGTERM, if this channel is empty, we can simply continue with the rest of graceful shutdown. Otherwise, we know there are goroutines that need interruption. 
2. The second channel is for communicating requesting graceful interruption to the streaming goroutines. The goroutine responsible for graceful shutdown will loop through messages in the in-progress channel, and for each will send a “request for an interruption” message. The streaming goroutines periodically listen to this channel, and if they receive a message, will initiate the graceful termination. 
