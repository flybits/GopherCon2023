...
For a sample demonstration please see [this](https://goplay.tools/snippet/-duJFuDAvhx) playground link. In the sample demonstration, we simulate a server that encounters two panics (on indexes 0 and 2) while iterating over 100 items. Our recover function resumes the iteration after each panic from the point of failure, so that the program completes "processing" all the iterations that did not cause a panic. The code snippet below summarizes the approach. The same idea and procedure also applies to the client, where it can recover from panics at each iteration (piece of stream data received) and continue. 
...
![image](https://user-images.githubusercontent.com/17835858/224375981-6e61d55b-4ebe-4c4f-ae90-8a36fcf8dfc0.png)


Please see [this](https://goplay.tools/snippet/ehadgGKvCZA) playground link for simulation of how the goroutine performing graceful shutdown can use the aforementioned channels to perform graceful interruption. Also, the code snippet below summurazies the approach.
