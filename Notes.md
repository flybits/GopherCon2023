...
1. The first channel `inProgressStreamingChannel` is to keep track of in-progress streaming goroutines. Upon receiving SIGTERM, if this channel is empty, we can simply continue with the rest of the graceful shutdown. Otherwise, we know there are goroutines that need interruption. 
